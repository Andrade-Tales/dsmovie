...
create table tb_score (value float8, movie-id int8 not null, user-id int8 not null, primary key (movie-id, user-id));
create table tb_user (id int8 generated by default as identity, email varchar(255), primary key (id));
alter table if exists tb_score add constraint FK5noqxp2awsnr0bfkcly5ljmr3 foreign key (movie-id) references tb_movie;
alter table if exists tb_score add constraint FKan4hfvc0f04q3xebg9cn6ofyc foreign key (user-id) references tb_user;
